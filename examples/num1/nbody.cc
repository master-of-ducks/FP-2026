...
int main ()
{
  ...
  // typedef RungeKutta4<Model> Solver;         // Solver type
  // Solver solver(model);                // instantiate solver
  ...
  solver.set_dt(1.0/64.0);             // set initial time step
  solver.set_TOL(1E-8);
  ...
  Number T = 50.0; // 100.0, figureeight: 2.1
  ...
  gnuplot("twobody_RK45.dat",times,states,dts); // output model result
