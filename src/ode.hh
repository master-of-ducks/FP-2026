...
/** @brief Generic Model Problem with Jacobian

    \tparam N a number type used in states and function values
    \tparam F lambda for f(t,x)
    \tparam F_x lambda for jacobian of f w.r.t. x
*/
template <typename N, typename F, typename F_x>
class GenericModel1 {
    // store components of IVP
    N _t0;
    Vector<N> _x0;
    F _f;
    F_x _f_x;
...
public:
    ...
    GenericModel1(const N& t0_, const Vector<N>& x0_, const F& f_,
                 const F_x& f_x_)
        : _t0(t0_), _x0(x0_), _f(f_), _f_x(f_x_) {}
    ...
    void initialize(N& t0, Vector<N>& x0) const {
        t0 = _t0;
        x0 = _x0;
    }
    ...
    void f(const N& t, const Vector<N>& x, Vector<N>& result) const {
        result = _f(t, x);
    }

    //! jacobian evaluation needed for implicit solvers
    // result needs to be allocated to the correct size
    void f_x(const N& t, const Vector<N>& x, DenseMatrix<N>& result) const {
        result = _f_x(t, x);
    }
};

/** @brief Generic Model Problem without Jacobian
...
public:
    ...
    //! constructor stores parameter lambda
    GenericModel2(const N& t0_, const Vector<N>& x0_, const F& f_)
        : _t0(t0_), _x0(x0_), _f(f_) {}
...
template <typename N, typename F, typename F_x>
GenericModel1<N, F, F_x> make_model(const N& t0, const Vector<N>& x0, const F& f,
                                   const F_x& f_x) 
{
    return GenericModel1<N, F, F_x>(t0, x0, f, f_x);
}

template <typename N, typename F>
GenericModel2<N, F> make_model(const N& t0, const Vector<N>& x0, const F& f)
{
    return GenericModel2<N, F>(t0, x0, f);
}
  ...
  */
  ...
  public:
    ...
    void step ()
    {
      ...
      time_type error_constant = error/pow(dt,0.2);
      time_type dt_opt1(dt*pow(rho*TOL/error,0.2));
      time_type dt_opt = std::min(beta*dt,std::max(alpha*dt,dt_opt1));
      // std::cout << "dt=" << dt << " est. error=" << error << " constant=" << error_constant << " dt_opt=" << dt_opt1 << " dt_sugg=" << dt_opt << std::endl;

      if (error<=TOL)
        {
          t += dt;
          u = ww;
          //if (std::abs(dt_opt/dt)<0.8 || std::abs(dt_opt/dt)>1.25)
            dt = dt_opt;
        }
      else
        {
          ...
          if (newton.has_converged())
            {
              ...
              if (!reduced && dt<dtmax-1e-13)
                {
                  dt = std::min(static_cast<time_type>(2.0)*dt,dtmax);
                  if (verbosity>0)
  ...
  private:
    ...
    static ButcherTableau initTableau(const std::string method)
    {
      if(method.find("Implicit Euler") != std::string::npos){
        ...
        const number_type alpha = 1. - sqrt(two)/2.;
        butcher[0][0] = alpha;
        butcher[0][1] = alpha;
        ...
        const number_type beta = 1./2./sqrt(three);
        butcher[0][0] = 0.5 + beta;
        butcher[0][1] = 0.5 + beta;

        butcher[1][0] = 0.5 - beta;
        butcher[1][1] = -1. / sqrt(three);
        ...
        const number_type two(2.0);
        const number_type theta = 1 - sqrt(two)/2.;
        const number_type alpha = 2. - sqrt(two);
