...
template <class T>
std::shared_ptr<const SparseMatrix<T>> calculateLUDecomposition(const std::shared_ptr<const SparseMatrix<T>> A, T drop_tol = 1e-12) {
        ...
        for (int i = k + 1; i < LU.rowsize(); ++i) {
            ...
            if (ik != LU[i].end()) {
                ...
                for (auto j = LU[k].find(i); j != LU[k].end(); ++j) {
                    ...
                    if (abs(a) > drop_tol) LU[i][j->first] -= a;
                    ++iterations;
                }  
            }
        } 
